...
// 백엔드 응답 타입 가정 (LoginResponse와 동일)
type LoginResponse = {
  id: number;
  name: string;
  role: "YOUTH" | "SENIOR";
...
const Auth = () => {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 🔥 로그인 정보 localStorage에 저장 → Header에서 읽어서 role/이름 사용
      ...
      // 메인으로 이동 → Header가 onnaUser 읽어서 UI 바뀜
      navigate("/");
    } catch (error: unknown) {
      const msg =
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "회원가입 완료",
        description: `${(res as LoginResponse).name}님, 이제 로그인해 주세요.`,
      });

      // 필요하면 로그인 탭으로 포커스 이동 로직 추가할 수 있음
      // ex) Tabs 상태 제어하거나 그대로 두고 로그인 탭 클릭 유도
    } catch (error: unknown) {
      const msg =
        error instanceof Error
          ? error.message
